...
def get_ahead_pred_obs(model, data, n_steps, dp):
    # dims
    T, dx = data.size()
    ...
    X_pred = tc.empty((n_steps, time_steps, dp), device=model.device)
...
def construct_ground_truth(data, n_steps, dg, do, dp):
    T, dx = data.size()
    time_steps = T - n_steps
    X_true = tc.empty((n_steps, time_steps, dp))
    for step in range(1, n_steps + 1):
        X_true[step - 1] = data[step : time_steps + step, dg+do:dg+do+dp]
    return X_true
...
@tc.no_grad()
def n_steps_ahead_pred_ppe(model, data, n_steps, dg, do, dp):
    x_pred = get_ahead_pred_obs(model, data, n_steps, dp)
    x_true = construct_ground_truth(data, n_steps, dg, do, dp).to(model.device)
